 ...
 */
public class ScBuyPenguinExp extends ScBase{
    
    private void buy(String uid, int nLoop, int exp)
                ...
                action.loadGame(uid, sessions, 
                    (Responder resp) -> 
                    {
                        ...
                        for(Map.Entry<String, Object> entry:cotes.entrySet())
                        {
                            ...
                            {
                                buyExpPenguin(nLoop, action, cote, uid, coteId, exp);
                            }
                        }
                    });
    ...
    private void buyExpPenguin(int loop, Actions action, Map<String, Object> cote, String uid, String coteId, int exp)
    {
        ...
        for(int i = 0; i < loop; ++i)
        {
            if(penguinList.size() > 0)
            {
                ...
                try {
                    action.buyPenguinExp(uid, coteId, peng_id, exp, sessions,
    ...
    public static void main(String[] args)
    {
        ...
        PGLogCalculator.getInst().setScenario("buy_penguin_exp");
        for(int i = 0; i < nTest; ++i)
        {
            ScBuyPenguinExp loader = new ScBuyPenguinExp();
            try {
                loader.buy("23885717", nLoop, 120);
            } catch (ClientStatusException | ServerStatusException ex) {
